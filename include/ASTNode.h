...
public:
    ...
    virtual IRExpr *emitIR(AbstractSyntaxTree *ast) { return NULL_IR_EXPR; }
    virtual Type_t checkType(AbstractSyntaxTree *ast) { return nullptr; }
...
public:
    IdentASTNode(const char *VALUE)
    {
        strcpy(_value, VALUE);
        kind = NK_IDENT_NODE;
    }

    IdentASTNode(const char *VALUE, int LineNum)
    {
        strcpy(_value, VALUE);
        lineNum = LineNum;
        kind = NK_IDENT_NODE;
    ...
    IRExpr *emitIR(AbstractSyntaxTree *ast);
    Type_t checkType(AbstractSyntaxTree *ast);
    void accept(TreeVisitor *v);
};
...
{
    char _value[1024]; // FIXME: This shouldn't be limited size value.
public:
    StringConstASTNode(const char *val)
    {
        strcpy(_value, val);
...
public:
    ...
    void setScope(Scope_t s) { _scope = s; }
    Scope_t getScope() { return _scope; }
...
public:
    AsmStmtASTNode(char *Data) { strcpy(_data, Data); }
...
public:
    ReturnStmtASTNode(ASTNode *Type, ASTNode *Expr) : _type(Type)
    {
        kind = NK_RETURN_STMT;
        ASSIGN_AST_NODE_REF(_expr, Expr);
    }

    ~ReturnStmtASTNode() { DELETE_AST_NODE_REF(_expr); }
...
public:
    LogNotExprASTNode(ASTNode *Type, ASTNode *Expr) : _type(Type)
    {
        kind = NK_LOG_NOT_EXPR;
        ASSIGN_AST_NODE_REF(_expr, Expr);
    }

    ~LogNotExprASTNode() { DELETE_AST_NODE_REF(_expr); }
...
public:
    CondExprASTNode(ASTNode *Condition, ASTNode *TrueBody, ASTNode *FalseBody)
    {
        kind = NK_COND_EXPR;
        ASSIGN_AST_NODE_REF(_condition, Condition);
        ASSIGN_AST_NODE_REF(_trueBody, TrueBody);
        ASSIGN_AST_NODE_REF(_falseBody, FalseBody);
    }

    ~CondExprASTNode()
    {
        DELETE_AST_NODE_REF(_condition);
        DELETE_AST_NODE_REF(_trueBody);
        DELETE_AST_NODE_REF(_falseBody);
    }
...
class GetCharExprASTNode : public ASTNode
{
    ASTNode *_expr;

public:
    GetCharExprASTNode(ASTNode *Expr)
    {
        kind = NK_GETC_EXPR;
        ASSIGN_AST_NODE_REF(_expr, Expr);
    }

    ~GetCharExprASTNode() { DELETE_AST_NODE_REF(_expr); }
...
class PutCharExprASTNode : public ASTNode
{
    ASTNode *_expr;

public:
    PutCharExprASTNode(ASTNode *Expr)
    {
        kind = NK_PUTC_EXPR;
        ASSIGN_AST_NODE_REF(_expr, Expr);
    }

    ~PutCharExprASTNode() { DELETE_AST_NODE_REF(_expr); }
...
class GetIntExprASTNode : public ASTNode
{
    ASTNode *_expr;

public:
    GetIntExprASTNode(ASTNode *Expr)
    {
        kind = NK_GETI_EXPR;
        ASSIGN_AST_NODE_REF(_expr, Expr);
    }

    ~GetIntExprASTNode() { DELETE_AST_NODE_REF(_expr); }
...
class PutIntExprASTNode : public ASTNode
{
    ASTNode *_expr;

public:
    PutIntExprASTNode(ASTNode *Expr)
    {
        kind = NK_PUTI_EXPR;
        ASSIGN_AST_NODE_REF(_expr, Expr);
    }

    ~PutIntExprASTNode() { DELETE_AST_NODE_REF(_expr); }
...
public:
    ListASTNode() { kind = NK_LIST; }

    ListASTNode(ASTNode *n)
    {
        kind = NK_LIST;
        add(n);
    }
    ...
    void add(ASTNode *n)
    {
        ...
        if (n->getKind() == NK_LIST)
        {
            std::vector<ASTNode *> &tmpVec =
                static_cast<ListASTNode *>(n)->getElements();
